...
void ControlBlock::allocateBufferFromPtr(std::string const &name, void *ptr,
                                         size_t length) {
  if (buf_map.find(name) != buf_map.end()) {
    throw std::runtime_error("Already registered buffer named " + name);
  }

  std::shared_ptr<uint8_t> data(reinterpret_cast<uint8_t *>(ptr),
                                [](uint8_t *) {});
  memset(data.get(), 0, length);
  ...
  buf_map.insert({name, index_length});
  LOGGER_INFO(logger, "Buffer '{}' of size {} used", name, length);
}
