...
class CertifierBuilder : public Builder<Certifier<CryptoScheme>> {
 public:
  CertifierBuilder(dory::ctrl::ControlBlock &cb, ProcId const local_id,
    ...
    for (auto const replica : replicas) {
      ...
      share_send_builders.emplace_back(
          ...
          tail, internal::ShareMessage<CryptoScheme>::BufferSize);
  ...
  ubft::certifier::Certifier<CryptoScheme> build() override {
    this->building();
    ...
    return Certifier<CryptoScheme>(crypto, thread_pool, tail, max_message_size, identifier,
