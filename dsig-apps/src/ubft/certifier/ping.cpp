...
int main(int argc, char *argv[]) {
  ...
  using Certificate = dory::ubft::certifier::Certificate<CertifierCrypto>;
  ...
  dory::ubft::certifier::CertifierBuilder<CertifierCrypto> ping_certifier_builder(
      cb, local_id, all_ids, "ping", crypto, thread_pool, tail, message_size);
  ping_certifier_builder.announceQps();
  dory::ubft::certifier::CertifierBuilder<CertifierCrypto> pong_certifier_builder(
  ...
  if (local_id == measurer_id) {
    ...
    ping_certifier.toggleFastPath(fast_path);
    pong_certifier.toggleFastPath(fast_path);
    ...
    for (size_t e = 0; e < experiments; e++) {
      if constexpr (std::is_same_v<CertifierCrypto, dory::ubft::Crypto::Dsig>) {
