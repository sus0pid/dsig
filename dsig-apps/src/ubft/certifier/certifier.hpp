...
class Certifier {
  ...
  using Share = internal::ShareMessage<CryptoScheme>;
  using ShareBuffer = typename Share::BufferLayout;
 ...
 public:
  using Certificate = dory::ubft::certifier::Certificate<CryptoScheme>;
  ...
  void acknowledge(Index const index, uint8_t const *const begin,
    ...
    while (computed_shares.size() + 1 > tail) {
      computed_shares.pop_front();
    }
  ...
  bool check(Certificate const &certificate) const {
    ...
    for (size_t i = 0; i < certificate.nbShares(); i++) {
      ...
      if (!CryptoScheme::crypto(crypto).verify(sig, hash->data(), hash->size(), signer)) {
 ...
 private:
  ...
  void offloadShareComputation() {
    ...
    for (auto &[i, buffer] : queued_share_computations) {
      ...
      //     [this, index, buffer = std::move(buffer)]() mutable {
            ...
            auto sign = CryptoScheme::crypto(crypto).sign(hash.data(), hash.size());
            computed_shares.emplace_back(
  ...
  void pollComputedShares() {
    ...
    while (!computed_shares.empty()) {
      std::optional<ComputedShare> computed_share{std::move(computed_shares.front())};
      computed_shares.pop_front();
    ...
    while (!msg_tail.empty() && !sorted_computed_shares.empty()) {
      ...
      for (auto &sender : share_senders) {
        auto *const slot = reinterpret_cast<ShareBuffer *>(
      ...
      *reinterpret_cast<ShareBuffer*>(opt_buffer->data()) =
  ...
  void enqueueShareVerification(Share &&share, size_t const replica) {
    ...
    //         [this, share = std::move(share), replica, hash = hash]() mutable {
              auto valid =
                  CryptoScheme::crypto(crypto).verify(share.signature(), hash.data(), hash.size(),
                                uat(share_receivers, replica).procId());
              verified_shares.emplace_back(
  ...
  void pollVerifiedShares() {
    ...
    // try_dequeue does not use the optional, hence the need for manual reset.
    while (!verified_shares.empty()) {
      // (verified_share.reset(), verified_shares.try_dequeue(verified_share))) {

      handleVerifiedShare(verified_shares.front());
      verified_shares.pop_front();
    }
  ...
  class MessageData {
   public:
    ...
    Certificate buildCertificate(Certifier const &certifier) const {
      ...
      std::vector<std::pair<ProcId, Signature const &>> signatures;
    ...
    bool signatureVerified(Certifier const &certifier, ProcId const source,
      ...
      for (auto const &[some_replica_index, some_share] : received_shares) {
        ...
        if (some_source == source) {
          ...
          return std::memcmp(&sig, &some_share.signature(), sizeof(sig)) == 0;
        }
      }
      return false;
    }
  ...
  size_t ticks = 0;
  std::deque<ComputedShare> computed_shares;
  // TODO(Antoine): have a per-replica queue.
  std::deque<VerifiedShare> verified_shares;
