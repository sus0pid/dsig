...
class Writer {
 ...
 public:
  ...
  thread_local
  inline static CachedHash cached = {false, 0, 0};
  void write(Index const index,
    ...
    if (!cached.cached || cached.index != index) {
      cached = {true, index, XXH3_64bits(&reg.header.incarnation, sizeof(Header::incarnation) + value_size)};
    }
    reg.header.hash = cached.hash;
