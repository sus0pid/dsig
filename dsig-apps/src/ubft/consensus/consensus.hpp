...
class Consensus {
 ...
 public:
  Consensus(TailThreadPool &thread_pool, tail_cb::Broadcaster &&cb_broadcaster,
            ...
            PrepareCertifier &&prepare_certifier,
            std::vector<tail_p2p::AsyncSender> &&fast_commit_senders,
            std::vector<tail_p2p::Receiver> &&fast_commit_receivers,
            std::vector<Certifier> &&vc_state_certifiers,
            Certifier &&checkpoint_certifier,
            std::vector<Certifier> &&cb_checkpoint_certifiers,
  ...
  void testApp(size_t const nb_proposals, size_t const request_size,
    ...
    dory::dsig::LatencyProfiler latency_profiler(fast_path ? 5000 : 100);
    ...
    while (true) {
      ...
      if (canPropose() && credits > 0 && proposed < accepted) {
        #if TCB_SCHEME==CRYPTO_SCHEME_Dsig
        if (proposed % 32 == 0) {
  ...
  void tick() {
    ...
    pollCbs();
    if (fast_path_enabled) pollPrepareCertificatePromises();
    ...
    if (fast_path_enabled) pollFastCommits();
  ...
  void toggleFastPath(bool const enable) {
    fast_path_enabled = enable;
    for (auto &receiver : cb_receivers) {
      receiver.toggleFastPath(enable);
    }
    prepare_certifier.toggleFastPath(enable);
    for (auto &certifier : vc_state_certifiers) {
      certifier.toggleFastPath(enable);
    }
  }
 ...
 private:
  ...
  bool isByzantineCommit(size_t from, PrepareCertificate &certificate) {
  ...
  void handleCommit(size_t from, CommitMessage &&commit) {
    ...
    auto certificate = std::move(std::get<PrepareCertificate>(certificate_ok));
    ...
    if (from != local_index) {
      ...
      //         [this, from, certificate = std::move(certificate)]() mutable {
                ...
                verified_commits.emplace_back(VerifiedCommit{from, std::move(certificate), valid});
              // });
    } else {
      // As the commit comes from us, we don't need to check it.
      verified_commits.emplace_back(VerifiedCommit{from, std::move(certificate), true});
    }
  }
  ...
  void pollVerifiedCommits() {
    ...
    while (!verified_commits.empty()) {
      auto &[from, certificate, valid] = verified_commits.front();
      if (unlikely(!valid)) {
   ...
   * @param certificate
   */
  void handleVerifiedCommit(size_t from, PrepareCertificate &&certificate) {
    auto &replica_state = uat(states, from);
  ...
  bool isByzantine(size_t from, NewViewMessage &new_view) {
    ...
    for (size_t i = 0; i < quorum; i++) {
      ...
      auto const certificate =
          std::get<Certificate>(std::move(certificate_ok));
  ...
  std::vector<TailQueue<PrepareCertificate>> buffered_commits;
  ...
  std::deque<VerifiedCommit> verified_commits;
