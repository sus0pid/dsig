...
struct CertificateMessage : public CbMessage {
 ...
 public:
  ...
  std::variant<std::invalid_argument, Certificate>
  tryIntoCertificate(
 ...
 private:
  ...
  size_t nbShares() const {
    return reinterpret_cast<typename Certificate::Header const *>(certificate())
        ->nb_shares;
  }
};

struct CommitMessage : public CertificateMessage<PrepareCertificate> {
 private:
  CommitMessage(tail_cb::Message &&msg) : CertificateMessage<PrepareCertificate>{std::move(msg)} {}

 public:
  size_t static bufferSize(size_t const proposal_size, size_t const nb_shares) {
    return CertificateOffset +
           PrepareCertificate::bufferSize(proposal_size, nb_shares);
  }
...
struct CheckpointMessage : public CertificateMessage<Certificate> {
 private:
  CheckpointMessage(tail_cb::Message &&msg)
      : CertificateMessage<Certificate>{std::move(msg)} {}
...
struct NewViewMessage : public CbMessage {
 ...
 public:
  ...
  SerializedState cloneSerializedState(size_t const index, size_t const window,
    ...
    auto certificate = std::get<Certificate>(
        Certificate::tryFrom(std::move(cert_buffer)));
