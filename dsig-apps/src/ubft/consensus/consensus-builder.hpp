...
class ConsensusBuilder : Builder<Consensus> {
  ...
  ConsensusBuilder(ctrl::ControlBlock &cb, ProcId const local_id,
    ...
    if (std::find(hosts.begin(), hosts.end(), local_id) != hosts.end()) {
      for (auto const broadcaster : replicas) {
        ...
        auto const hash_ns =
            fmt::format("hash-consensus-{}-cb-{}", identifier, broadcaster);
        ...
        for (auto const writer : receivers) {
          ...
          host_builders.emplace_back(cb, local_id, writer, receivers, hash_ns,
                                     cb_tail,
                                     tail_cb::Receiver::HashRegisterValueSize);
        }
      }
    }
    ...
    for (auto const replica : without(replicas, local_id)) {
      ...
      cb_checkpoint_senders_builders.emplace_back(
          ...
          Certificate::bufferSize(max_cb_checkpoint_size,
                                             replicas.size() / 2 + 1));
 ...
 private:
  ...
  PrepareCertifierBuilder prepare_certifier_builder;
  ...
  std::vector<CertifierBuilder> vc_state_certifier_builders;
  // We need to certify checkpoints.
  CertifierBuilder checkpoint_certifier_builder;
  // We need to certify all cb checkpoints.
  std::vector<CertifierBuilder> cb_checkpoint_certifier_builders;
