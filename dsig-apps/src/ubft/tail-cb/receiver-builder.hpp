...
class ReceiverBuilder : Builder<Receiver> {
 public:
  ReceiverBuilder(dory::ctrl::ControlBlock &cb, ProcId const local_id,
      ...
      : message_recv_builder{cb,
        ...
        writer_builder{cb,         local_id, hosts_ids,
                       ...
                       true},
        hash_writer_builder{cb,         local_id, hosts_ids,
                       fmt::format("hash-{}", identifier), tail,     Receiver::HashRegisterValueSize,
                       true},
    ...
    for (auto const receiver_id : receivers_ids) {
      ...
      hash_reader_builders.emplace_back(cb, local_id, receiver_id, hosts_ids,
                                   fmt::format("hash-{}", identifier), tail,
                                   Receiver::HashRegisterValueSize);
    }
  }
  ...
  void announceQps() override {
    ...
    for (auto &builder : hash_reader_builders) {
      builder.announceQps();
    }
    hash_writer_builder.announceQps();
  }
  ...
  void connectQps() override {
    ...
    hash_writer_builder.connectQps();
  }
  ...
  Receiver build() override {
    ...
    std::vector<replicated_swmr::Reader> hash_readers;
    for (auto &builder : hash_reader_builders) {
      hash_readers.push_back(builder.build());
    }
    ...
    return Receiver(crypto, thread_pool, broadcaster_id, borrowed_messages,
                    ...
                    writer_builder.build(), std::move(hash_readers), hash_writer_builder.build());
  }
 ...
 private:
  ...
  std::vector<replicated_swmr::ReaderBuilder> hash_reader_builders;
  replicated_swmr::WriterBuilder hash_writer_builder;
  ProcId const broadcaster_id;
  Crypto &crypto;
