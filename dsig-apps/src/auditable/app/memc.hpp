...
class Memc : public Application {
 public:
    ...
    if (server) {
      ...
      if (!memc_rpc->connect()) {
        throw std::runtime_error(
            "Failed to connect to the local memc instance");
      }
  ...
  size_t maxRequestSize() const {
    auto const max_put_size =
        kvstores::memcached::put_max_buffer_size(key_size, value_size);
    auto const max_get_size =
        kvstores::memcached::get_max_buffer_size(key_size);
  ...
  void execute(uint8_t const *const request, size_t request_size,
    ...
    // std::cout << "Request: " << kvstores::buff_repr(request, request +
    ...
    if (received.size() > 0) {
      ...
      // std::cout << "Response: " << kvstores::buff_repr(response.begin(),
 ...
 private:
  ...
  void prepare_requests() {
    ...
    for (size_t i = 0; i < prepared_requests_cnt; i++) {
      ...
      kvstores::memcached::put(req.data(), keys[circular_index % unique_keys],
                               value_size);
      prepared_requests.push_back(req);
      circular_index++;
    }
