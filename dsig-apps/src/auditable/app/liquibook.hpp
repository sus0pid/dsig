...
class Liquibook : public Application {
 public:
  ...
  size_t maxRequestSize() const { return sizeof(ClientRequest); }
  ...
  void execute(uint8_t const *const request, size_t request_size,
    ...
    auto immediately_filled =
        trader.placeOrder(req->req_id, req->is_buy, req->price, req->qty);

    // Predict the size of the response
    size_t client_responses = 0;
    if (trader.previousResponsesNum() > 0) {
      client_responses =
          std::min(trader.previousResponsesNum(), max_num_cli_resp);
    }

    response.resize(cli_resp_offset +
                    client_responses * sizeof(ClientResponse));
    ...
    if (trader.previousResponsesNum() > 0) {
      auto *cli_resp = reinterpret_cast<ClientResponse *>(
          reinterpret_cast<uintptr_t>(resp) + cli_resp_offset);
      resp->cli_resp.num = trader.copyPreviousResponses(
          static_cast<int>(client_responses), cli_resp);
    }
  }
  ...
  static std::string resp_buff_repr(InputIt first, InputIt last) {
    ...
    ss << "[";
    auto *cli_resp = reinterpret_cast<ClientResponse *>(
        reinterpret_cast<uintptr_t>(r) + cli_resp_offset);
