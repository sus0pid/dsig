...
class Flip : public Application {
 public:
  ...
  size_t maxRequestSize() const { return max_request_size; }

  size_t maxResponseSize() const { return max_request_size; }
 ...
 private:
  ...
  void prepare_requests() {
    for (size_t i = 0; i < prepared_requests_cnt; i++) {
      prepared_requests.push_back(
          random_string(min_request_size, max_request_size));
    }
  }
