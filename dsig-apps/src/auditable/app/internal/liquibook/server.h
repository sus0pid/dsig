...
class TraderContext {
 public:
  TraderContext(orderentry::Market *market,
                orderentry::NamedOrderBook &namedBook)
      : market_(market), namedBook_(namedBook) {}

  bool placeOrder(uint64_t req_id, bool buy_otherwise_sell,
                  liquibook::book::Quantity qty, liquibook::book::Price price) {
    return market_->placeOrder(namedBook_, notifier, req_id, buy_otherwise_sell,
                               qty, price);
  }

  size_t previousResponsesNum() { return notifier.size(); }
