...
struct ClientResponses {
  int num;
  ptrdiff_t offset;  // The offset where the replies start from the beginning of
                     // the replication Response
};

struct ReplicationResponse {
  int server_id;
  enum Kind { OK, CHANGE_LEADER, FATAL };
...
static constexpr uintptr_t cli_resp_offset =
    round_up_powerof2<16>(sizeof(ReplicationResponse));
