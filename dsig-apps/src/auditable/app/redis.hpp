...
class Redis : public Application {
 public:
  Redis(bool server, std::string const &config_string) {
    ...
    if (server) {
      int redis_port = 9997;  // TODO: + local_id;
      ...
      if (!redis_rpc->connect()) {
        throw std::runtime_error(
            "Failed to connect to the local redis instance");
      }
  ...
  size_t maxRequestSize() const {
    auto const max_put_size =
        kvstores::redis::put_max_buffer_size(key_size, value_size);
 ...
 private:
  ...
  void prepare_requests() {
    ...
    for (size_t i = 0; i < prepared_requests_cnt; i++) {
      ...
      kvstores::redis::put(req.data(), keys[circular_index % unique_keys],
                           value_size);
      prepared_requests.push_back(req);
      circular_index++;
    }
