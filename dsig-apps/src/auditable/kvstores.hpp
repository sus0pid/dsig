...
static void spawn_memc(int port) {
  ...
  if (env == nullptr) {
    throw std::runtime_error("Environment variable " +
                             std::string(libreparent) + " not set");
  }

  std::filesystem::path path(env);
  if (!path.is_absolute()) {
    throw std::runtime_error(std::string(env) + " must be an absolute path");
  }

  if (!std::ifstream(env).good()) {
    throw std::runtime_error(std::string(env) + " does not exist");
  }
  ...
  if (child_pid == 0) {
    ...
    char const *envs[] = {ld_preload.c_str(), NULL};
...
static size_t put(void *buffer, std::vector<uint8_t> const &key,
...
static void spawn_redis(int port) {
  ...
  pid_t child_pid = fork();
  if (child_pid == 0) {
    char const *args[] = {"/usr/bin/redis-server",
                          "--save",
                          "\"\"",
                          "--appendonly",
                          "no",
                          "--port",
                          fmt::format("{}", port).c_str(),
                          NULL};
    char const *envs[] = {ld_preload.c_str(), NULL};
...
static size_t put(void *buffer, std::vector<uint8_t> const &key,
  ...
  cur += std::sprintf(reinterpret_cast<char *>(buf + cur), "%d",
